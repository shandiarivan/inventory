...
$pdf->Cell(50,5,'Laporan Barang Keluar','0','1','L',false);
$pdf->Ln(3);

$pdf->SetFont('Arial','B',8.5);
$pdf->Cell(8,6,'No.',1,0,'C');
$pdf->Cell(30,6,'Tgl Keluar',1,0,'C');
$pdf->Cell(30,6,'Kode Barang',1,0,'C');
$pdf->Cell(35,6,'Nama Barang',1,0,'C');
$pdf->Cell(25,6,'Jml Keluar',1,0,'C');
...
if(isset($_GET['filter']) && ! empty($_GET['filter'])){ 
    ...
    if($filter == '1'){ 
        ...
        $query = "SELECT * FROM tbl_lap_bk WHERE DATE(tgl_keluar)='".$_GET['tanggal']."' "; 
    }else if($filter == '2'){ 
        $nama_bulan = array('', 'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember');

        echo '<b>Data Transaksi Bulan '.$nama_bulan[$_GET['bulan']].' '.$_GET['tahun'].'</b><br /><br />';

        $query = "SELECT * FROM tbl_lap_bk WHERE MONTH(tgl_keluar)='".$_GET['bulan']."' AND YEAR(tgl_keluar)='".$_GET['tahun']."' "; 
    }else if($filter == '3'){ 
        echo '<b>Data Transaksi Tahun '.$_GET['tahun'].'</b><br /><br />';

        $query = "SELECT * FROM tbl_lap_bk WHERE YEAR(tgl_keluar)='".$_GET['tahun']."' "; 
    }else{

        echo '<b>Data Transaksi Periode '.$_GET['dari'].'  '.$_GET['sampai'].'</b>';

        $query = "SELECT * FROM tbl_lap_bk WHERE DATE(tgl_keluar) BETWEEN '".$_GET['dari']."' AND '".$_GET['sampai']."'  ";
    }
}else{ // Jika user tidak memilih filter
    echo '<b>Semua Data Transaksi</b><br /><br />';

    $query = "SELECT * FROM tbl_lap_bk ORDER BY id_lap DESC "; 
}
...
if($row > 0){
    while ($data = mysqli_fetch_array($sql)) {
        ...
        $pdf->Cell(30,4,$data['tgl_keluar'],1,0,'L');
        $pdf->Cell(30,4,$data['kode_barang'],1,0,'L');
        $pdf->Cell(35,4,$data['nama_barang'],1,0,'L');
        $pdf->Cell(25,4,$data['jml_keluar'],1,0,'C');
        $pdf->Cell(30,4,$data['customer'],1,0,'L');
