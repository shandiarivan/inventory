...
$pdf->Cell(50,5,'Laporan Stok Masuk','0','1','L',false);
$pdf->Ln(3);
...
if(isset($_GET['filter']) && ! empty($_GET['filter'])){ 
    ...
    if($filter == '1'){ 
        ...
        $query = "SELECT * FROM tbl_lap_sm WHERE DATE(tgl_masuk)='".$_GET['tanggal']."' "; 
    }else if($filter == '2'){ 
        $nama_bulan = array('', 'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember');

        echo '<b>Data Transaksi Bulan '.$nama_bulan[$_GET['bulan']].' '.$_GET['tahun'].'</b><br /><br />';

        $query = "SELECT * FROM tbl_lap_sm WHERE MONTH(tgl_masuk)='".$_GET['bulan']."' AND YEAR(tgl_masuk)='".$_GET['tahun']."' "; 
    }else if($filter == '3'){ 
        echo '<b>Data Transaksi Tahun '.$_GET['tahun'].'</b><br /><br />';

        $query = "SELECT * FROM tbl_lap_sm WHERE YEAR(tgl_masuk)='".$_GET['tahun']."' "; 
    }else{

        echo '<b>Data Transaksi Periode '.$_GET['dari'].'  '.$_GET['sampai'].'</b>';

        $query = "SELECT * FROM tbl_lap_sm WHERE DATE(tgl_masuk) BETWEEN '".$_GET['dari']."' AND '".$_GET['sampai']."'  ";
    }
}else{ // Jika user tidak memilih filter
    echo '<b>Semua Data Transaksi</b><br /><br />';

    $query = "SELECT * FROM tbl_lap_sm ORDER BY id_lap DESC "; 
}
